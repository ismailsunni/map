...
// ── Map ───────────────────────────────────────────────────────
const cityCenter = fromLonLat([110.3695, -7.7956])
...
async function loadLandmarks() {
  try {
    landmarks = await rpc('get_yogyakarta_landmarks')
...
document.getElementById('go-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    const [geoData, distData] = await Promise.all([
      rpc('get_yogyakarta_vertex_route', { from_vertex_id: routeFrom.vertexId, to_vertex_id: routeTo.vertexId }),
      rpc('get_yogyakarta_random_distances', { vertex_ids: [routeFrom.vertexId, routeTo.vertexId] }),
    ])
    setProgress('rt', 90)
...
async function runTSP() {
  ...
  try {
    ...
    const distances = await rpc('get_yogyakarta_random_distances', { vertex_ids: vids })
    setProgress('tsp', 45)
    ...
    for (let k = 0; k < total; k++) {
      ...
      try {
        const geoData = await rpc('get_yogyakarta_vertex_route', { from_vertex_id: vids[loop[k]], to_vertex_id: vids[loop[k + 1]] })
        const features = new GeoJSON().readFeatures(parseGeo(geoData), { dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857' })
...
map.on('click', async (e) => {
  if (currentMode === 'route') {
    ...
    if (pickingFor) {
      ...
      try {
        const res = await rpc('get_yogyakarta_nearest_vertex', { lat, lon })
